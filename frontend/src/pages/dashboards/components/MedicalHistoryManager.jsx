...
const MedicalHistoryManager = ({ conditions }) => {
  const [history, setHistory] = useState([]);

  useEffect(() => {
    // If conditions passed as prop (string or array)
    if (conditions) {
        if (Array.isArray(conditions)) {
            setHistory(conditions.map(c => ({ condition: c, status: 'Active', date_diagnosed: new Date().toISOString() })));
        } else if (typeof conditions === 'string') {
            // Split by comma if string
            const list = conditions.split(',').map(s => s.trim()).filter(s => s);
            setHistory(list.map(c => ({ condition: c, status: 'Active', date_diagnosed: new Date().toISOString() })));
        }
    } else {
        setHistory([]);
    }
