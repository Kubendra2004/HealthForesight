...
const PatientChat = ({ patientId }) => {
  ...
  const baseInputRef = useRef(""); // For real-time voice
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const aiMsg = {
        id: Date.now() + 1,
        text: res.data.response, // Backend now returns friendly message on key missing
        sender: 'ai',
        timestamp: new Date(),
        metadata: {
      ...
      let errorText = "I'm having trouble connecting to the server. Please check your connection.";
      
      if (error.response) {
          if(error.response.status === 500) {
              errorText = "I'm experiencing an internal error. Please try again later or contact support.";
        ...
        >
                ...
                <AnimatePresence>
                    ...
                    {loading && (
                        <motion.div 
                            ...
                            animate={{ opacity: 1 }} 
                            exit={{ opacity: 0 }}
                            style={{ display: 'flex', justifyContent: 'flex-start', marginBottom: '1rem', paddingLeft: '4.5rem' }}
                        >
            ...
            <Box sx={{ p: 3, bgcolor: 'white', borderTop: '1px solid #e2e8f0' }}>
                <Box sx={{ display: 'flex', gap: 2 }}>
                    ...
                    <VoiceInput 
                         onListenStart={() => {
                             baseInputRef.current = input; // Snapshot current text
                         }}
                         onLive={(liveText) => {
