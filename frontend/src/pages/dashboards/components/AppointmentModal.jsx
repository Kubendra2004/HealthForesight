...
const AppointmentModal = ({ open, onClose, onSubmit, doctors }) => {
  ...
  return (
    <Dialog open={open} onClose={loading ? null : onClose} maxWidth="sm" fullWidth>
       ...
       <DialogContent sx={{ p: 4, mt: 1, minHeight: '400px' }}>
         <AnimatePresence mode="wait">
           ...
           {step === 2 && (
             <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
                style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', paddingTop: '10px' }}
             >
                <TextField 
                  select 
                  label="Select Doctor" 
                ...
                >
                  ...
                  {doctors.map(doc => (
                    <MenuItem key={doc.id || doc.username} value={doc.id || doc.username}>Dr. {doc.name || doc.username} ({doc.specialization || 'General'})</MenuItem>
                  ))}
