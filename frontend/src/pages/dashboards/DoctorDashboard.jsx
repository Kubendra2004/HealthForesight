    ...
    const handleIssue = async () => {
        ...
        try {
            const payload = {
                ...
                instructions: notes ? `${notes} (${dosage})` : `${dosage}`,
        ...
        <Grid container spacing={4} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12} md={6}>
                <Paper sx={{ ...glassCard, p: 4, height: '100%', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <Box display="flex" alignItems="flex-start" gap={1} mb={4}>
                        <TextField 
                            fullWidth 
                            ...
                            sx={{ bgcolor: 'white', borderRadius: 2 }}
                        />
                        <VoiceInput onSpeechEnd={(text) => setNotes(prev => (prev + ' ' + text).trim())} />
                    </Box>
...
const AppointmentsTab = ({ token, user, onChatOpen }) => {
    ...
    const missedAppts = appointments.filter(a => new Date(a.date) < todayObj && a.status !== 'Completed' && a.status !== 'Cancelled');
    
    // Combine for display
    const requestsAndMissed = [...requestedAppts, ...missedAppts];
            ...
            <Paper sx={{ ...glassCard, p: 3, mb: 2 }}>
                <Grid container spacing={2} alignItems="center">
                    ...
                    <Grid item xs={12} md={3}>
                        ...
                        />
                        <Chip 
                            label={new Date(appt.date) < new Date() && appt.status !== 'Completed' && appt.status !== 'Cancelled' ? 'Missed / Overdue' : appt.status} 
                            size="small"
                            color={appt.status === 'Scheduled' ? 'success' : appt.status === 'Requested' ? 'warning' : 'error'}
                            sx={{ ml: 1 }}
                        />
                    </Grid>
                    <Grid item xs={12} md={3}>
                        ...
                        {appt.status === 'Scheduled' && (
                            <Box display="flex" gap={1}>
                                ...
                                <IconButton 
                                    color="primary" 
                                    onClick={() => onChatOpen && onChatOpen(appt.patient_id)}
                                >
                                    <EmailRounded />
                                </IconButton>
                            </Box>
    ...
    return (
        <Box>
            ...
            <Tabs value={tabValue} onChange={(e, v) => setTabValue(v)} sx={{ mb: 3 }}>
                ...
                <Tab label={`Requests / Missed (${requestedAppts.length + missedAppts.length})`} />
            </Tabs>
            ...
            {tabValue === 2 && (
                requestsAndMissed.length === 0 ? (
                    <Paper sx={{ ...glassCard, p: 4, textAlign: 'center' }}>
                        <Typography variant="h6" color="text.secondary">No pending requests or missed appointments</Typography>
                    </Paper>
                ) : requestsAndMissed.map(appt => <AppointmentCard key={appt._id} appt={appt} />)
            )}
        </Box>
    );
};
  ...
  const [chatPartner, setChatPartner] = useState(null);
    ...
    useEffect(() => {
        ...
        if (lastMessage?.type === 'new_appointment') {
           console.log("⚡ Received Appointment:", lastMessage.data);
           setSnackbar({ 
             open: true, 
             msg: `New Appointment Request from Patient ${lastMessage.data.patient_id}`, 
             type: 'info' 
           });
           fetchData();
        } else if (lastMessage?.type === 'new_message') {
            // Only notify if chat is NOT open with this user
            // But user asked for "pop up", so maybe always show?
            // "give a pop up notification when a chat is done from either side"
            // Let's show it always for now as a confirmation
            setSnackbar({
                open: true,
    ...
    }, [user, navigate, token, lastMessage]);
                ...
                {activeTab === 'appointments' && (
                    <motion.div key="appointments" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
                        <AppointmentsTab 
                            token={token} 
                            user={user} 
                            onChatOpen={(patientId) => {
                                setChatPartner({ username: patientId });
                                setChatOpen(true);
                            }}
                        />
                    </motion.div>
