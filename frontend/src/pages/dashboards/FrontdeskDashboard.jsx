...
const FrontdeskDashboard = () => {
  ...
  const [assignForm, setAssignForm] = useState({ patient_id: '', doctor_id: '' });
  ...
  useEffect(() => {
    const timeoutId = setTimeout(async () => {
        try {
            ...
            if (res.ok) {
                setPatientOptions(await res.json());
            }
        } catch (error) {
            console.error(error);
        }
  ...
  useEffect(() => {
      if (lastMessage?.type === 'bed_update') {
          console.log("⚡ Received Bed Update:", lastMessage.data);
          const { bed_id, status } = lastMessage.data;
          
          setBeds(prevBeds => prevBeds.map(bed => 
              bed._id === bed_id ? { ...bed, status } : bed
          ));
          
          // Optional: Show snackbar
          setSnackbar({ 
              open: true, 
              message: `Bed ${status} update received!`, 
              severity: 'info' 
          });
      }
  }, [lastMessage]);
  ...
  const handleAssignPatient = async () => {
      try {
          const res = await fetch(`http://localhost:8000/frontdesk/assign_patient`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
          ...
          if (res.ok) {
               if (data.message.includes("already assigned")) {
                   ...
                   setAssignForm({ patient_id: '', doctor_id: '' });
               }
          } else {
    ...
    return (
      <Box>
        ...
        <Grid container spacing={2} mb={4}>
          {[
            { label: 'Assign Patient', icon: PersonAddRounded, color: '#ec4899', onClick: () => setAssignmentModal(true) },
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f0f4ff 0%, #e0e7ff 100%)', pb: 12 }}>
        ...
        <DialogContent>
          <Box display="flex" flexDirection="column" gap={2} mt={1}>
             ...
             <Box display="flex" alignItems="flex-start" gap={1}>
                ...
                <VoiceInput onSpeechEnd={(text) => setApptForm({...apptForm, reason: (apptForm.reason + ' ' + text).trim()})} />
             </Box>
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={bedModal} onClose={() => setBedModal(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
            ...
            <Autocomplete
                ...
                getOptionLabel={(option) => typeof option === 'string' ? option : option.label || ''}
                filterOptions={(x) => x} 
                value={patientOptions.find(p => p.username === bedForm.patient_id) || null} 
                onChange={(event, newValue) => {
                    const pid = newValue ? (newValue.username || newValue.id) : '';
                    setBedForm({...bedForm, patient_id: pid});
                }}
                onInputChange={(event, newInputValue) => setPatientQuery(newInputValue)}
                renderInput={(params) => <TextField {...params} label="Search Patient" fullWidth autoFocus />}
            />
        </DialogContent>
        <DialogActions>
      ...
      {/* Assignment Modal */}
      <Dialog open={assignmentModal} onClose={() => setAssignmentModal(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Assign Patient to Doctor</DialogTitle>
        <DialogContent>
            <Typography variant="body2" color="text.secondary" mb={3}>
                Create a long-term link between a patient and a doctor (e.g. Primary Care).
            </Typography>
            <Box display="flex" flexDirection="column" gap={3}>
                <Autocomplete
                    ...
                    onChange={(event, newValue) => {
                        const pid = newValue ? (newValue.username || newValue.id) : '';
                        setAssignForm({...assignForm, patient_id: pid});
                    }}
                    onInputChange={(event, newInputValue) => setPatientQuery(newInputValue)}
        ...
        <DialogActions>
            <Button onClick={() => setAssignmentModal(false)}>Cancel</Button>
            <Button variant="contained" onClick={handleAssignPatient} disabled={!assignForm.patient_id || !assignForm.doctor_id}>Assign</Button>
        </DialogActions>
      </Dialog>
